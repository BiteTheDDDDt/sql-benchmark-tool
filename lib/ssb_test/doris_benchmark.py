...
class DorisBenchmark(object):
    ...
    def check_results(self, sql_dirname, scale):
        ...
        try:
            ...
            selector=[]
            #selector=[40,49,75,78,84]
            # selector=[i for i in range(38,50)]
            # selector=[]
            pass_num=0
            ...
            for sql_dir in test_sql_dirs:
                ...
                for sql_dict in sql_list:
                    ...
                    for line in query_res["result"]:
                        ...
                        normalize_line = [col[:-2] if col.endswith(".0") else col for col in normalize_line]
                        normalize_line_str = "\t".join(normalize_line)

                        input_result.append(normalize_line_str)

                    for i in range(0,len(base_result)):
                        base_result[i]=base_result[i].replace("\\N","NULL")
                    ...
                    is_same = True
                    for line in query_res["result"]:
                        normalize_line_str = input_result[i]
                        base_result_line_str = base_result[i]
                        
                        base_result_line_str=base_result_line_str.replace("\\N","NULL")
